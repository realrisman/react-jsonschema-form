// Originally from
// https://github.com/DefinitelyTyped/DefinitelyTyped/blob/master/types/@realrisman/rjsf-core/index.d.ts

declare module '@realrisman/rjsf-core' {
import * as React from 'react';
import {JSONSchema7, JSONSchema7Definition, JSONSchema7Type, JSONSchema7TypeName} from 'json-schema';

  type ErrorSchema = { [k: string]: ErrorSchema; };
  ...
  export type UiSchema = {
    ...
    'ui:ObjectFieldTemplate'?: React
                                .StatelessComponent<ObjectFieldTemplateProps>;
    [name: string]: any;
  };

  export type FieldId = {$id: string;};

  export type IdSchema<T = any> = {[key in keyof T]: IdSchema<T[key]>;}&FieldId;

  export type FieldPath = {$name: string;};

  export type PathSchema<T = any> =
  {[key in keyof T]: PathSchema<T[key]>;}&FieldPath;
  ...
  export type Widget =
      React.StatelessComponent<WidgetProps>|React.ComponentClass<WidgetProps>;

  export interface FieldProps<T = any> extends Pick<
      React.HTMLAttributes<HTMLElement>,
      Exclude<keyof React.HTMLAttributes<HTMLElement>, 'onBlur'>> {
    schema: JSONSchema7;
    uiSchema: UiSchema;
    idSchema: IdSchema;
    formData: T;
    ...
    onBlur: (id: string, value: boolean|number|string|null) => void;
    registry: {
      fields: {[name: string]: Field}; widgets: {[name: string]: Widget};
  ...
  export type Field =
      React.StatelessComponent<FieldProps>|React.ComponentClass<FieldProps>;

  export type FieldTemplateProps = {
    id: string; classNames: string; label: string;
  ...
  export type ArrayFieldTemplateProps<T = any> = {
    DescriptionField: React.StatelessComponent<
        {id: string; description: string | React.ReactElement}>;
    TitleField:
        React
            .StatelessComponent<{id: string; title: string; required: boolean}>;
    canAdd: boolean;
    className: string;
    disabled: boolean;
    idSchema: IdSchema;
  ...
  export type ObjectFieldTemplateProps<T = any> = {
    ...
    description: string;
    properties: {
      content: React.ReactElement; name: string; disabled: boolean;
      readonly: boolean;
    }[];
    required: boolean;
    schema: JSONSchema7;
  ...
  export type AjvError = {
    message: string; name: string; params: any; property: string; stack: string;
  };

  export type ErrorListProps = {
    errorSchema: FormValidation; errors: AjvError[]; formContext: any;
    schema: JSONSchema7;
    uiSchema: UiSchema;
  };
  ...
  type FieldValidation = {
    __errors: FieldError[]; addError: (message: string) => void;
  };
  ...
  type FormSubmit<T = any> = { formData: T; };
  ...
  export module utils {
    ...
    export function mergeObjects(
        obj1: object, obj2: object, concatArrays?: boolean): object;
    ...
    export function toConstant(schema: JSONSchema7): JSONSchema7Type|
        JSONSchema7['const']|Error;

    export function isSelect(
        _schema: JSONSchema7,
        definitions?: FieldProps['registry']['definitions']): boolean;
    ...
    export function shouldRender(
        comp: React.Component, nextProps: any, nextState: any): boolean;
    ...
    export function parseDateString(
        dateString: string, includeTime?: boolean): DateObject;

    export function toDateString(
        dateObject: DateObject, time?: boolean): string;

    export function pad(num: number, size: number): string;

    export function setState(
        instance: React.Component, state: any, callback: Function): void;
...
declare module '@realrisman/rjsf-core/lib/components/fields/SchemaField' {
import {FieldProps} from '@realrisman/rjsf-core';
...
declare module '@realrisman/rjsf-core/lib/validate' {
import {JSONSchema7Definition} from 'json-schema';
import {AjvError, ErrorSchema, FormProps} from '@realrisman/rjsf-core';
